...
def plotting(y_t, y_h,input_size,occupancy_index,ii,horizon):
    ...
    for h in range(horizon):
        ...
        mse = mean_squared_error(y_t[:,h], y_h[:,h])
        print('week'+str(ii)+'__room'+str(room)+'__h'+str(h)+':')
    ...
    result.to_csv('allfeatures-T_Z_room'+str(room)+'_horizon'+str(horizon) + '_week'+str(ii) + '.csv', index=True)
...
for w in [0, 1]:
    ...
    for room in [1,2,4,5,6,7]:
        for horizon in [1,2,5,10,20,30]:
            ...
            model=TransformerModel(input_dim=input_size, hidden_dim= 32,output_size = horizon, n_layers = num_layers, n_heads = nhead, dropout=0.5)
            ...
            torch.save(model, 'horizon'+str(horizon)+'room'+str(room)+'.pth')
