...
for w in [0,1]:
    ...
    for room in [1,2,4,5,6,7]:
        for horizon in [1,2,5,10,20,30]:
            ...
            # print(class_weights, trainY.max(), trainY.min())
            # model = tree.DecisionTreeClassifier(class_weight=class_weights)

            from sklearn.neural_network import MLPClassifier

            model = MLPClassifier(hidden_layer_sizes = (32, horizon))

            # import autosklearn.classification
            #
            # model = autosklearn.classification.AutoSklearnClassifier(time_left_for_this_task=1 * 60,
